...
async function getServicesStatus() {
  ...
  services.apis.openai = process.env.OPENAI_API_KEY
    ? "available"
    : "unavailable";
  services.apis.perplexity = process.env.PERPLEXITY_API_KEY
    ? "available"
    : "unavailable";
  ...
  try {
    ...
    const averageLatency =
      last24Hours.length > 0
        ? last24Hours.reduce((sum, r) => sum + r.totalLatencyMs, 0) /
          last24Hours.length
        : null;

    const errorRate =
      last24Hours.length > 0
        ? last24Hours.filter((r) => r.stages.some((s) => !s.success)).length /
          last24Hours.length
        : 0;
...
async function getAuditMetrics() {
  try {
    ...
    const failedAuthAttempts = last24Hours.filter(
      (log) => log.action === "auth_failed",
    ).length;

    const rateLimitHits = last24Hours.filter(
