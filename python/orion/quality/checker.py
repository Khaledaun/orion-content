...
def check_quality(article_text: str, metadata: dict, primary_keyword: str, 
    ...
    threshold = enforcement.get('default_min_quality_score', 80)
    ...
    if not passed:
        issues.append({
            ...
            "severity": "high",
            "message": f"Quality score ({final_score}) below threshold ({threshold})",
            "suggestion": "Address other issues to improve overall quality"
        })
    ...
    return {
        ...
        "breakdown": {
            ...
            "seo": seo_score,
            "aio": aio_score,
            "ai_search_visibility": ai_search_score
        },
        "issues": issues,
        ...
        "enforcement": {
            "passed": passed,
            "threshold": threshold,
            "tag_if_below": enforcement.get('tag_if_below', 'review-needed') if not passed else None
        }
    }
