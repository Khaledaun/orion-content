...
async function seedRulebook() {
  ...
  try {
    ...
    if (existingRulebook) {
      console.log(
        `✅ Rulebook already exists (version ${existingRulebook.version})`,
      );
    ...
    await prisma.rulebookVersion.create({
      data: {
        ...
        sources: DEFAULT_SOURCES,
        notes:
          "Initial Global Standard Rulebook (GSRB) - Conservative baseline for content quality",
      },
    });
    ...
    console.log(
      `   Quality threshold: ${DEFAULT_RULEBOOK.enforcement.default_min_quality_score}`,
    );
    console.log(
      `   E-E-A-T weight: ${DEFAULT_RULEBOOK.score_weights.eeat * 100}%`,
    );
    ...
    if (verification) {
      ...
      console.error(
        "❌ Verification failed - Rulebook not found after creation",
      );
      process.exit(1);
    }
  } catch (error) {
