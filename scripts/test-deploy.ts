...
class TestDeployScript {
  ...
  constructor() {
    ...
    this.environment =
      process.env.PRODUCTION === "1" ? "production" : "staging";
  ...
  private async runTests(): Promise<void> {
    ...
    const testSuites = [
      ...
      {
        name: "Unit Tests",
        command: "npx jest __tests__ --verbose --passWithNoTests",
      },
      {
        name: "Integration Tests",
        command:
          "npx jest --testNamePattern=integration --verbose --passWithNoTests",
      },
    ];
  ...
  private async buildApplication(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `  ✅ Build completed successfully (${Math.round(buildTime / 1000)}s)`,
      );
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
    ...
    try {
      ...
      for (const [planType, account] of Object.entries(this.testAccounts)) {
        try {
          ...
          console.log(
            `  ✅ ${planType.toUpperCase()} account: ${account.email}`,
          );
        } catch (error) {
          console.log(
            `  ℹ️  ${planType.toUpperCase()} account may already exist, skipping...`,
          );
        }
      }
      ...
      console.log(
        `  ⚠️  Seeding failed: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  private async validateEndpoints(): Promise<void> {
    ...
    if (failedEndpoints.length > 0) {
      ...
      console.log(
        "  ℹ️  This may be expected if endpoints are not yet implemented",
      );
    }
  ...
  private async validateEndpoint(endpoint: string): Promise<void> {
    ...
    try {
      ...
      if (success) {
        console.log(
          `  ✅ ${endpoint}: HTTP ${response.status} (${responseTime}ms)`,
        );
      } else {
        console.log(
          `  ❌ ${endpoint}: HTTP ${response.status} (${responseTime}ms)`,
  ...
  private printQAChecklist(): void {
    ...
    Object.entries(this.testAccounts).forEach(([planType, account]) => {
      console.log(
        `  □ Login with ${planType.toUpperCase()} account: ${account.email}`,
      );
    });
    console.log("");
