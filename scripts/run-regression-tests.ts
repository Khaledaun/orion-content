...
class RegressionTestRunner {
  ...
  private async runCoreApiTests(): Promise<TestSuite> {
    ...
    // API route tests
    tests.push(
      await this.runTest("Rulebook API Route", async () => {
        const response = await fetch(`${this.baseUrl}/api/rulebook`, {
          method: "POST",
        });
    ...
    tests.push(
      await this.runTest("Strategy API Route", async () => {
        const response = await fetch(`${this.baseUrl}/api/sites/123/strategy`, {
          method: "POST",
        });
        if (response.status !== 401)
          throw new Error(`Expected 401, got ${response.status}`);
    ...
    // Ops API tests
    tests.push(
      await this.runTest("Ops Status Route", async () => {
  ...
  private async runSecurityTests(): Promise<TestSuite> {
    ...
    // Authentication test
    tests.push(
      await this.runTest("Authentication Required", async () => {
        const endpoints = [
          "/api/rulebook",
          "/api/sites/1/strategy",
          "/api/ops/status",
        ];
  ...
  private async runDatabaseTests(): Promise<TestSuite> {
    ...
    tests.push(
      await this.runTest("Rulebook Seeding", async () => {
        try {
          const output = execSync("npx tsx scripts/seed_rulebook.ts", {
            encoding: "utf8",
            timeout: 15000,
          });
  ...
  private async runObservabilityTests(): Promise<TestSuite> {
    ...
    tests.push(
      await this.runTest("Observability Module", async () => {
        const { ProductionObservabilityTracker } = await import(
          "../lib/observability-prod"
        );
        const tracker = new ProductionObservabilityTracker(
          "test-123",
          "site-test",
          "Test Content",
        );
  ...
  private async runI18nTests(): Promise<TestSuite> {
    ...
    // Extended i18n test
    tests.push(
      await this.runTest("Extended i18n Support", async () => {
        const {
          generateExtendedSlug,
          formatExtendedCitation,
          validateExtendedI18nCompliance,
        } = await import("../lib/i18n-extended");

        // Test French
        const frenchSlug = generateExtendedSlug(
        ...
        const arabicCitation = formatExtendedCitation(
          "مصدر",
          "عنوان",
          "http://example.com",
          "ar",
