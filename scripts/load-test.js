...
class LoadTestHarness {
  ...
  async processSite(siteId) {
    ...
    siteResult.summary.averageLatency =
      successfulRequests.length > 0
        ? successfulRequests.reduce((sum, r) => sum + r.latency, 0) /
          successfulRequests.length
        : 0;

    console.log(
      `✅ Site ${siteId} completed: ${siteResult.summary.successful}/${siteResult.summary.total} successful`,
    );
    return siteResult;
  }
  ...
  async makeRequest(siteId, requestId) {
    ...
    try {
      const response = await fetch(
        `${CONFIG.baseUrl}/api/sites/${siteId}/strategy`,
        {
          method: "POST",
          headers: {
  ...
  calculateSummary() {
    ...
    const allErrors = allRequests.filter((r) => r.error).map((r) => r.error);
  ...
  generateReport() {
    ...
    console.log(
      `   • Total Duration: ${(this.results.totalDurationMs / 1000).toFixed(2)}s`,
    );
    ...
    console.log(
      `   • Average Latency: ${s.averageLatency?.toFixed(2) || "N/A"}ms`,
    );
    console.log(
      `   • Min/Max Latency: ${s.minLatency?.toFixed(2) || "N/A"}ms / ${s.maxLatency?.toFixed(2) || "N/A"}ms`,
    );
    ...
    console.log(
      `   • Other Failures: ${s.failedRequests - s.rateLimitHits - s.authFailures}`,
    );
...
async function runPerformanceBenchmarks() {
  ...
  Object.entries(benchmarks).forEach(([name, result]) => {
    console.log(
      `   ${name}: ${result.averageLatency.toFixed(2)}ms avg (${result.samples} samples)`,
    );
  });
