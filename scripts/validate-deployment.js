...
try {
  ...
  if (packageJson.packageManager) {
    warnings.push('packageManager field found in package.json - should be removed for npm usage');
  }
...
try {
  ...
  if (!vercelJson.buildCommand || !vercelJson.buildCommand.includes('prisma generate')) {
...
try {
  ...
  if (!nextConfig.includes('webpack')) {
    warnings.push('next.config.js missing webpack configuration for path aliases');
  }
...
try {
  ...
  if (middleware.includes('enhanced-middleware')) {
    errors.push('middleware.ts still imports enhanced-middleware which has Edge Runtime issues');
  }
  
  if (middleware.includes('process.')) {
    errors.push('middleware.ts contains Node.js APIs incompatible with Edge Runtime');
  }
