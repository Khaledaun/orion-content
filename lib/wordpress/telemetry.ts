...
export class WordPressTelemetry {
   ...
   */
  async trackPublishingEvent(event: Omit<WordPressPublishingEvent, "id" | "timestamp">): Promise<void> {
    try {
   ...
   */
  async getSiteMetrics(siteId: string, days: number = 30): Promise<WordPressSiteMetrics | null> {
    try {
      ...
      return {
        ...
        averagePublishTime: events.length > 0 ? totalPublishTime / events.length : 0,
        totalCost,
        lastPublishAt: events[0]?.timestamp || null,
        connectionUptime,
        averageQualityScore: events.length > 0 ? totalQualityScore / events.length : 0,
      };
   ...
   */
  async getGlobalMetrics(days: number = 30): Promise<WordPressPublishingMetrics> {
    try {
      const since = new Date();
      since.setDate(since.getDate() - days);
      ...
      return {
        ...
        costPerSuccessfulPublish: successfulEvents.length > 0 ? totalCost / successfulEvents.length : 0,
        ...
        qualityScoreAverage: events.length > 0 ? totalQualityScore / events.length : 0,
      };
   ...
   */
  async getPublishingTrends(days: number = 30): Promise<Array<{
    date: string;
    publishes: number;
    successfulPublishes: number;
    failedPublishes: number;
    ...
    try {
      ...
      const trends = Array.from(eventsByDate.entries()).map(([date, dayEvents]) => {
        ...
        return {
          ...
          totalCost,
          averageQualityScore: dayEvents.length > 0 ? totalQualityScore / dayEvents.length : 0,
        };
      });
   ...
   */
  private async storeEventInRedis(event: WordPressPublishingEvent): Promise<void> {
    ...
    try {
      ...
      const currentCost = parseFloat((await this.redisStore.get(`${eventKey}:cost`)) || "0");
      await this.redisStore.set(`${eventKey}:cost`, (currentCost + event.cost).toString(), 7 * 24 * 3600);
   ...
   */
  private async storeEventInDatabase(event: WordPressPublishingEvent): Promise<void> {
    try {
   ...
   * Update site-specific metrics
   */
  private async updateSiteMetrics(event: WordPressPublishingEvent): Promise<void> {
    try {
      const today = new Date().toISOString().substring(0, 10);
      ...
      // Update cost tracking
      const currentCost = parseFloat((await this.redisStore.get(`${siteKey}:cost`)) || "0");
      await this.redisStore.set(`${siteKey}:cost`, (currentCost + event.cost).toString(), 30 * 24 * 3600);

      // Update quality score tracking
      const currentQualitySum = parseFloat((await this.redisStore.get(`${siteKey}:quality_sum`)) || "0");
      const currentQualityCount = parseInt((await this.redisStore.get(`${siteKey}:quality_count`)) || "0");
      
      await this.redisStore.set(`${siteKey}:quality_sum`, (currentQualitySum + event.qualityScore).toString(), 30 * 24 * 3600);
