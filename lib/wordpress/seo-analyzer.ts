...
export class WordPressSEOAnalyzer {
  ...
  private extractSEOPluginInfo(
    page: CrawlResult,
  ): WordPressSEOInfo["seoPlugins"] {
  ...
  private categorizePlugin(
    pluginName: string,
  ): "seo" | "performance" | "security" | "other" {
  ...
  private isSEOOptimizedTheme(page: CrawlResult): boolean {
    ...
    const hasSchemaMarkup = page.scripts.some(
      (script) => script.includes("schema.org") || script.includes("json-ld"),
    );

    return (
      hasProperHeadings && hasMetaDescription && hasTitle && hasSchemaMarkup
    );
  }
  ...
  private isSEOPluginConfigured(
    page: CrawlResult,
    pluginName: string,
  ): boolean {
    // Basic check - if plugin is detected and page has good SEO elements, assume configured
    const hasGoodSEO =
      !!page.title && !!page.metaDescription && page.headings.h1.length === 1;
    return hasGoodSEO;
  }
  ...
  private checkSchemaMarkup(
    page: CrawlResult,
  ): WordPressSEOInfo["schemaMarkup"] {
    const hasSchema = page.scripts.some(
      (script) => script.includes("schema.org") || script.includes("json-ld"),
    );
  ...
  private generateRecommendations(
    ...
    if (info.seoPlugins.length === 0) {
      critical.push(
        "Install a WordPress SEO plugin like Yoast SEO or RankMath",
      );
    }
    ...
    if (
      info.permalinkStructure === "Plain" ||
      info.permalinkStructure === "Numeric"
    ) {
      important.push(
        'Change permalink structure to "Post name" for better SEO',
      );
    }
