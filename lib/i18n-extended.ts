...
export function generateExtendedSlug(
  title: string,
  lang: string = "en",
): string {
...
function formatEnglishCitation(
  source: string,
  title: string,
  url: string,
  author?: string,
...
function getDefaultAltText(lang: string, imageType?: string): string {
  ...
  return (
    langDefaults[imageType as keyof typeof langDefaults] || langDefaults.default
  );
}
...
export function validateExtendedI18nCompliance(content: {
  ...
  if (content.lang === "ar" || content.lang === "he") {
    if (!content.slug.includes(`article-${content.lang}-`)) {
      issues.push(
        `${content.lang.toUpperCase()} content should use hash-based slug format`,
      );
    }
  ...
  if (rtlLangs.includes(content.lang)) {
    const hasRTLCitations = content.citations.some(
      (cite) =>
        cite.includes("\u202E") ||
    ...
    if (!hasRTLCitations && content.citations.length > 0) {
      ...
      suggestions.push(
        `Use formatExtendedCitation() with lang="${content.lang}"`,
      );
    }
  }
  ...
  const prefix =
    expectedAltPrefix[content.lang as keyof typeof expectedAltPrefix];
  if (prefix && content.altTexts.length > 0) {
    const hasCorrectFormat = content.altTexts.some((alt) =>
      alt.includes(prefix),
    );
    if (!hasCorrectFormat) {
      issues.push(`Alt texts should be in ${content.lang} language`);
      suggestions.push(
        `Use generateExtendedAltText() with lang="${content.lang}"`,
      );
    }
  }
  ...
  if (rtlLangs.includes(content.lang) && !schema.textDirection) {
    suggestions.push(
      'Consider adding textDirection="rtl" to schema for RTL languages',
    );
  }
...
export function processMixedScriptContent(
  content: string,
  primaryLang: string,
): {
  content: string;
