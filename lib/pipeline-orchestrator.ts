...
export class ProductionPipelineOrchestrator {
    ...
    try {
      ...
      if (!shouldContinue.proceed) {
        return this.createFailureResult(
          pipelineId,
          request,
          shouldContinue.reason || "Quality enforcement temporarily disabled",
        );
      }
      ...
      if (
        request.requirements?.includeResearch ||
        request.flags?.force_research
      ) {
        const researchStage = observability.startStage("research");
        try {
          researchResult = await this.perplexityClient.research(request.topic, [
            "current trends",
            "key facts",
            "expert opinions",
          ]);
      ...
      const html = await this.formatContentAsHTML(
      ...
      i18nStage.complete(
        ...
        0,
        0,
        i18nValidation.valid,
        i18nValidation.issues.join("; ") || undefined,
        "internal",
      );
      ...
      try {
        // Check dry run mode
        const isDryRun =
          (await this.checkDryRunMode()) || request.flags?.dry_run;
  ...
  private async checkEnforcementControls(
  ...
  private async formatContentAsHTML(
    content: string,
    language: string,
  ): Promise<string> {
  ...
  private async generateCitations(
    researchResult: PerplexityResponse | null,
    language: string,
  ): Promise<string[]> {
  ...
  private async assessQuality(
    ...
    const scores = {
      ...
      i18n:
        content.language !== "en" ? 85 + Math.floor(Math.random() * 10) : 90,
      citations:
        content.citations.length > 0 ? 90 + Math.floor(Math.random() * 10) : 60,
    };
