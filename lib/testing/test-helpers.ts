    ...
    for (const table of tableOrder) {
      try {
        if (typeof (this.prisma as any)[table]?.deleteMany === "function") {
          ...
          console.warn(
            `Prisma model for table ${table} does not exist or does not support deleteMany`,
          );
        }
      } catch (error) {
...
export class ApiTestUtils {
  public static createMockRequest(
    options: {
      method?: string;
      url?: string;
...
export class PerformanceTestUtils {
  ...
  public static expectPerformance(
    ...
    if (executionTime > maxTimeMs) {
      console.warn(
        `Performance warning: ${operation} took ${executionTime}ms (max: ${maxTimeMs}ms)`,
      );
    }
