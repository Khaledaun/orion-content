...
export class PerformanceMonitor {
  ...
  public endTest(testName: string): PerformanceReport | null {
    ...
    const memoryUsage: NodeJS.MemoryUsage = {
      ...
      arrayBuffers:
        currentMemory.arrayBuffers - testData.startMemory.arrayBuffers,
    };
  ...
  private calculateResourceUtilization(memoryUsage: NodeJS.MemoryUsage): {
    cpu: number;
    memory: number;
  } {
    ...
    const memoryUtilization =
      (memoryUsage.heapUsed / (1024 * 1024 * 1024)) * 100; // % of 1GB
  ...
  private checkThresholdViolations(
    ...
    if (durationThreshold && duration > durationThreshold.maxValue) {
      violations.push(
        `Test duration (${duration}ms) exceeded threshold (${durationThreshold.maxValue}ms)`,
      );
    }
    ...
    for (const metric of metrics) {
      ...
      if (threshold && metric.value > threshold.maxValue) {
        violations.push(
          `${metric.name} (${metric.value}${metric.unit}) exceeded threshold (${threshold.maxValue}${threshold.unit})`,
        );
      }
    }
