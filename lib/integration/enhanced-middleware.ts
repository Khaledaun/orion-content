...
export async function enhancedMiddleware(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const requestScope = serviceContainer.createScope(`req_${Date.now()}_${Math.random()}`)
...
export function setupEnhancedMiddleware(): void {
  ...
  middlewareStack.use('database-health', async (request, context, next) => {
    ...
    order: 5,
    conditions: [
      { type: 'path', matcher: /^\/api\//, negate: false }
    ]
  })

  // Performance monitoring middleware
  middlewareStack.use('performance-monitoring', async (request, context, next) => {
  ...
  middlewareStack.use('error-handling', async (request, context, next) => {
    try {
      return await next()
    } catch (error) {
  ...
  middlewareStack.use('service-container', async (request, context, next) => {
