...
export class EnvironmentManager {
  ...
  public validateConfiguration(): {
    ...
    if (this.isProduction()) {
      if (!this.config.security.encryptionKey || this.config.security.encryptionKey.length < 32) {
        errors.push('Production requires strong encryption key (32+ characters)')
      }
      ...
      if (!this.config.auth.secret || this.config.auth.secret === 'development-secret') {
        errors.push('Production requires secure auth secret')
      }
    }

    // Database validation
    if (!this.config.database.url.includes('sslmode=require') && this.isProduction()) {
  ...
  public generateConfigReport(): string {
    ...
    for (const rule of this.validationRules) {
      const value = process.env[rule.key]
      const status = value ? '✅' : '❌'
      const displayValue = rule.sensitive && value ? '[REDACTED]' : (value || '[NOT SET]')
  ...
  private setupValidationRules(): void {
    this.validationRules = [
      ...
      // Authentication
      { key: 'NEXTAUTH_SECRET', required: true, type: 'string', sensitive: true },
      { key: 'NEXTAUTH_URL', required: true, type: 'url' },
      
      // Security
      { key: 'ENCRYPTION_KEY', required: true, type: 'string', sensitive: true },
      { key: 'JWT_SECRET', required: false, type: 'string', sensitive: true },
      
      // API Keys
      { key: 'OPENAI_API_KEY', required: false, type: 'string', sensitive: true },
      { key: 'GOOGLE_API_KEY', required: false, type: 'string', sensitive: true },
      
      // Optional services
      { key: 'REDIS_URL', required: false, type: 'url', sensitive: true },
      
      // Monitoring
      { key: 'LOG_LEVEL', required: false, type: 'string',
...
export const getBoolEnv = (key: string, defaultValue: boolean = false): boolean => {
  const value = process.env[key]
