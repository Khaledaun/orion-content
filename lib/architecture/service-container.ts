...
export class ServiceContainer {
  ...
  public register<T>(
    ...
    const definition: ServiceDefinition<T> = {
      ...
      lifecycle: options.lifecycle || (options.singleton !== false ? 'singleton' : 'transient'),
      tags: options.tags || []
    }
  ...
  public async resolve<T>(name: string, scope?: string): Promise<T> {
    ...
    switch (definition.lifecycle) {
      ...
      default:
        throw new Error(`Unknown lifecycle '${definition.lifecycle}' for service '${name}'`)
    }
  }
  ...
  public getServiceInfo(): Array<ServiceMetadata & { definition: Omit<ServiceDefinition, 'factory'> }> {
    const info: Array<ServiceMetadata & { definition: Omit<ServiceDefinition, 'factory'> }> = []
  ...
  private async resolveScoped<T>(name: string, definition: ServiceDefinition, scope: string): Promise<T> {
  ...
  private async createInstance<T>(name: string, definition: ServiceDefinition, scope?: string): Promise<T> {
    try {
