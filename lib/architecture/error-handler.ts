...
export class ErrorHandler {
  ...
  public registerRecoveryStrategy(strategy: ErrorRecoveryStrategy): ErrorHandler {
  ...
  private updateErrorStats(errorInfo: ErrorInfo): void {
    ...
    const stats = this.errorStats.get(errorKey) || { count: 0, lastOccurrence: new Date() }
  ...
  private async attemptRecovery(error: Error, context: ErrorContext): Promise<NextResponse | null> {
  ...
  private createErrorResponse(errorInfo: ErrorInfo): NextResponse {
    ...
    const responseBody = {
      error: {
        message: errorInfo.statusCode >= 500 && !isDevelopment 
  ...
  private async enhanceError(error: Error, context?: Partial<ErrorContext>): Promise<Error> {
