...
export class MiddlewareStack {
  ...
  public async process(request: NextRequest): Promise<NextResponse> {
    ...
    try {
      ...
      const chain = this.createMiddlewareChain(applicableMiddlewares, request, context)
  ...
  private getApplicableMiddlewares(request: NextRequest): MiddlewareDefinition[] {
  ...
  private evaluateCondition(condition: MiddlewareCondition, request: NextRequest): boolean {
    let matches = false
  ...
  private setupBuiltinMiddlewares(): void {
    ...
    this.use('security-headers', async (request, context, next) => {
    ...
    this.use('request-id', async (request, context, next) => {
      const response = await next()
    ...
    // CORS Middleware
    this.use('cors', async (request, context, next) => {
    ...
    this.use('performance', async (request, context, next) => {
