...
export class ProductionObservabilityTracker {
  ...
  async finalize(
    ...
    const totalCost = this.stages.reduce(
      (sum, stage) => sum + stage.costUsd,
      0,
    );
    const totalTokens = this.stages.reduce(
      (sum, stage) => sum + stage.tokensInput + stage.tokensOutput,
      0,
    );
  ...
  private async updateCostMetrics(report: ObservabilityReport): Promise<void> {
    ...
    try {
      ...
      const currentCostStr =
        (await this.redisStore.get(`${costKey}:total`)) || "0";
      ...
      report.stages.forEach((stage) => {
        if (stage.provider) {
          providerCosts[stage.provider] =
            (providerCosts[stage.provider] || 0) + stage.costUsd;
        }
      });
...
export async function getCostMetrics(days: number = 7): Promise<CostMetrics> {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const dayCost = parseFloat(
      (await redisStore.get(`${dayKey}:total`)) || "0",
    );
    totalCost += dayCost;

    const dayTokens = parseInt(
      (await redisStore.get(`${dayKey}:tokens`)) || "0",
    );
    totalTokens += dayTokens;

    // Provider costs
    for (const provider of ["openai", "perplexity"]) {
      const providerCost = parseFloat(
        (await redisStore.get(`${dayKey}:provider:${provider}`)) || "0",
      );
