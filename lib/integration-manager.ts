...
export class IntegrationManager {
    ...
    try {
      ...
      switch (type) {
        case IntegrationType.WORDPRESS:
          testResult = await this.testWordPressConnection(
            credentials as WordPressCredentials,
          );
          break;
        case IntegrationType.GSC:
          testResult = await this.testGSCConnection(
            credentials as GSCCredentials,
          );
          break;
        case IntegrationType.GA4:
          testResult = await this.testGA4Connection(
            credentials as GA4Credentials,
          );
          break;
        case IntegrationType.OPENAI:
          testResult = await this.testOpenAIConnection(
            credentials as OpenAICredentials,
          );
          break;
        case IntegrationType.PERPLEXITY:
          testResult = await this.testPerplexityConnection(
            credentials as PerplexityCredentials,
          );
          break;
        default:
  ...
  private async testGSCConnection(
  ...
  private async testPerplexityConnection(
    creds: PerplexityCredentials,
  ): Promise<{ success: boolean; message: string }> {
  ...
  static generateDummyCredentials(
    type: IntegrationType,
