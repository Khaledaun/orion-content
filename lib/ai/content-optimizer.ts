...
export class ContentOptimizer {
  ...
  async optimizeContent(
    ...
    try {
      ...
      const seoOptimizations = this.applySEOOptimizations(
        optimizedContent,
        analysis,
        targetKeywords,
      );
      ...
      const readabilityOptimizations = this.applyReadabilityOptimizations(
        optimizedContent,
        analysis,
      );
      ...
      const engagementOptimizations = this.applyEngagementOptimizations(
        optimizedContent,
        analysis,
      );
      ...
      const structureOptimizations = this.applyStructureOptimizations(
        optimizedContent,
        analysis,
      );
      ...
      const keywordOptimizations = this.applyKeywordOptimizations(
        optimizedContent,
        analysis,
        targetKeywords,
      );
      ...
      const linkOptimizations = this.applyLinkOptimizations(
        optimizedContent,
        analysis,
      );
      ...
      logger.info(
        {
          ...
          overallImprovement: improvements.overallImprovement,
        },
  ...
  private analyzeSEO(
    ...
    if (title.length < 30) {
      issues.push({
        ...
        implementation:
          "Add descriptive words to make the title longer and more compelling",
      });
      score -= 15;
    ...
    if (metaDescription.length < 120) {
      issues.push({
        ...
        effort: "low",
        examples: [
          `"${metaDescription}" → "${metaDescription} Learn more about this topic and discover valuable insights."`,
        ],
      ...
      issues.push({
        ...
        implementation:
          "Shorten the meta description while keeping key information",
      });
      score -= 5;
    } else {
    ...
    if (keywords.length > 0) {
      ...
      if (keywordDensity < 0.01) {
        issues.push({
          ...
          implementation:
            "Include the primary keyword more naturally throughout the content",
        });
        score -= 20;
        ...
        issues.push({
          ...
          implementation:
            "Use keyword variations and synonyms to avoid over-optimization",
        });
        score -= 15;
      } else {
  ...
  private analyzeReadability(
    content: string,
  ): ContentAnalysis["analysis"]["readability"] {
    const issues: ContentOptimizationSuggestion[] = [];
    ...
    if (avgSentenceLength > 20) {
      issues.push({
        ...
        implementation:
          "Split complex sentences into simpler, more digestible parts",
      });
      score -= 15;
    } else {
    ...
    if (gradeLevel > 12) {
      issues.push({
        ...
        description:
          "Content is written at a college level, which may be too complex for general audience",
        ...
        implementation:
          "Simplify vocabulary and sentence structure for broader accessibility",
      });
      score -= 20;
    } else {
      improvements.push(
  ...
  private analyzeEngagement(
    content: string,
    title: string,
  ): ContentAnalysis["analysis"]["engagement"] {
    ...
    if (headlineQuality < 70) {
      suggestions.push({
        ...
        implementation:
          "Rewrite headline to be more compelling and attention-grabbing",
      });
      score -= 20;
    }
    ...
    if (wordCount < 300) {
      suggestions.push({
        ...
        implementation:
          "Expand content with more valuable information and examples",
      });
      score -= 15;
    }
    ...
    if (imageCount === 0) {
      suggestions.push({
        ...
        implementation:
          "Include relevant images and visual elements to break up text",
      });
      score -= 10;
    }
    ...
    // Call to action
    const hasCTA =
      /(click here|learn more|read more|get started|sign up|download|buy now)/i.test(
        content,
      );
    if (!hasCTA) {
      suggestions.push({
        ...
        implementation:
          "Include a clear call to action to guide reader behavior",
      });
      score -= 10;
    }
  ...
  private analyzeStructure(
    content: string,
  ): ContentAnalysis["analysis"]["structure"] {
    const issues: ContentOptimizationSuggestion[] = [];
    let score = 100;
    ...
    if (lists < 2) {
      issues.push({
        ...
        effort: "low",
        examples: [
          "Convert some paragraphs to bullet points or numbered lists",
        ],
        implementation:
          "Use lists to present information in a scannable format",
      });
      score -= 5;
    }
  ...
  private analyzeKeywords(
    content: string,
    keywords: string[],
  ): ContentAnalysis["analysis"]["keyword"] {
    const issues: ContentOptimizationSuggestion[] = [];
    let score = 100;
        ...
        issues: [
          {
            ...
            implementation:
              "Define primary and secondary keywords for the content",
          },
        ],
      };
    }
    ...
    if (density < 0.01) {
      issues.push({
        ...
        examples: [`Add "${primaryKeyword}" naturally in the content`],
        implementation:
          "Include the primary keyword more frequently throughout the content",
      });
      score -= 20;
    }

    // Keyword placement
    const titleHasKeyword = content
      .toLowerCase()
      .includes(primaryKeyword.toLowerCase());
    const firstParagraphHasKeyword = content
      .split("\n")[0]
      .toLowerCase()
      .includes(primaryKeyword.toLowerCase());
    ...
    return {
      ...
      density,
      placement:
        (titleHasKeyword ? 50 : 0) + (firstParagraphHasKeyword ? 50 : 0),
  ...
  private analyzeLinks(content: string): ContentAnalysis["analysis"]["link"] {
    ...
    const anchorTexts = (content.match(/\[([^\]]+)\]/g) || []).map((match) =>
      match.slice(1, -1),
    );
    ...
    if (internalLinks < 2) {
      issues.push({
        ...
        description:
          "Content needs more internal links for better site structure",
    ...
    const genericAnchors = anchorTexts.filter((text) =>
      ["click here", "read more", "here", "this", "link"].includes(
        text.toLowerCase(),
      ),
    );
    ...
    if (genericAnchors.length > 0) {
      issues.push({
        ...
        implementation:
          "Replace generic anchor text with descriptive, keyword-rich text",
      });
      score -= 10;
    }
  ...
  private generateRecommendations(
    ...
    if (highPriority.length > 0) {
      recommendations.push(
        `Address ${highPriority.length} high-priority issues first`,
      );
    }

    if (mediumPriority.length > 0) {
      recommendations.push(
        `Focus on ${mediumPriority.length} medium-priority improvements`,
      );
    }

    recommendations.push(
      "Review all suggestions and implement based on your content strategy",
    );
  ...
  private applySEOOptimizations(
    ...
    const seoSuggestions = analysis.analysis.seo.issues.filter(
      (s) => s.priority === "high",
    );
    ...
    seoSuggestions.forEach((suggestion) => {
      if (
        suggestion.type === "seo" &&
        suggestion.title.includes("keyword density")
      ) {
  ...
  private applyReadabilityOptimizations(
    ...
    const readabilitySuggestions = analysis.analysis.readability.issues.filter(
      (s) => s.priority === "high",
    );
  ...
  private applyEngagementOptimizations(
    ...
    const engagementSuggestions =
      analysis.analysis.engagement.suggestions.filter((s) =>
        s.title.includes("call to action"),
      );
  ...
  private applyStructureOptimizations(
    ...
    const structureSuggestions = analysis.analysis.structure.issues.filter(
      (s) => s.title.includes("headings"),
    );
  ...
  private applyKeywordOptimizations(
    ...
    // Apply keyword optimizations
    const keywordSuggestions = analysis.analysis.keyword.issues.filter(
      (s) => s.priority === "high",
    );
  ...
  private applyLinkOptimizations(
    ...
    const linkSuggestions = analysis.analysis.link.issues.filter((s) =>
      s.title.includes("internal links"),
    );
  ...
  private calculateImprovements(
    ...
    const overallImprovement = Math.floor(
      (seoImprovement + readabilityImprovement + engagementImprovement) / 3,
    );
  ...
  private estimateImpact(
    ...
    const rankingImprovement = Math.min(
      10,
      Math.floor(improvements.overallImprovement / 5),
    );
  ...
  private getSEORequirements(
    contentType: string,
  ): ContentTemplate["seoRequirements"] {
    return {
  ...
  private getReadabilityTargets(
    contentType: string,
  ): ContentTemplate["readabilityTargets"] {
    return {
