  ...
  private async makeRequest(
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `10Web API error: ${response.status} ${response.statusText}`,
          );
        }
  ...
  async registerSite(siteData: {
    ...
    return await this.makeRequest("/sites", {
      ...
      body: JSON.stringify({
        ...
        url:
          siteData.url ||
          `https://${siteData.name.toLowerCase().replace(/\s+/g, "-")}.com`,
    ...
    const response = await this.makeRequest(`/sites/${tenWebSiteId}/sync`, {
      ...
      body: JSON.stringify({
        ...
        content: {
          ...
          content:
            "<p>This is a test draft created by Orion Content Management System.</p>",
          status: "draft",
