...
class SEOAuditTools {
  ...
  private async performAudit(url: string): Promise<SEOAuditResult> {
    ...
    if (!hasMetaDescription) {
      issues.push({
        ...
        impact: 'high',
        howToFix: 'Add a compelling meta description between 150-160 characters that summarizes the page content'
      })
    }
    ...
    if (!hasProperHeadings) {
      issues.push({
        ...
        impact: 'medium',
        howToFix: 'Ensure proper heading hierarchy (H1 → H2 → H3) and include target keywords'
      })
    }
    ...
    if (!hasImageAltTags) {
      issues.push({
        ...
        impact: 'medium',
        howToFix: 'Add descriptive alt text to all images for better accessibility and SEO'
      })
    }
    ...
    if (loadTime > 2.5) {
      issues.push({
        ...
        impact: 'high',
        howToFix: 'Optimize images, minify CSS/JS, enable compression, and consider a CDN'
      })
    }
    ...
    recommendations.push({
      ...
      description: 'Add more contextual internal links to improve site navigation and SEO',
    ...
    recommendations.push({
      ...
      description: 'Add schema markup to help search engines understand your content',
  ...
  async batchAudit(urls: string[]): Promise<SEOAuditResult[]> {
    ...
    logger.info(`Completed batch audit: ${results.length}/${urls.length} successful`)
    return results
  }
