...
export class SEOAuditEngine {
  ...
  private async enhanceWithWordPressData(
    ...
    if (!hasSEOPlugins) {
      wordpressIssues.push({
        ...
        description:
          "No WordPress SEO plugins were detected. Consider installing Yoast SEO or RankMath.",
        impact: "medium",
  ...
  compareAudits(
    ...
    Object.keys(current.score).forEach((category) => {
      ...
      const currentScore =
        current.score[category as keyof typeof current.score];
      const previousScore =
        previous.score[category as keyof typeof previous.score];
      ...
      if (change > 5) {
        ...
        regressions.push(
          `${category} score decreased by ${Math.abs(change)} points`,
        );
      }
    });
