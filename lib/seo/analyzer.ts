...
export class SEOAnalyzer {
  ...
  private calculateScores(issues: SEOIssue[], pages: CrawlResult[]): SEOScore {
    ...
    issues.forEach((issue) => {
      ...
      const impactMultiplier =
        issue.impact === "high" ? 0.3 : issue.impact === "medium" ? 0.6 : 0.8;
  ...
  private generateRecommendations(
    ...
    if (score.technical < 70) {
      recommendations.push(
        "Improve technical SEO by fixing broken links and site structure issues.",
      );
    }

    if (score.content < 70) {
      recommendations.push(
        "Enhance content quality by adding missing meta descriptions and optimizing titles.",
      );
    }

    if (score.performance < 70) {
      recommendations.push(
        "Optimize page speed by compressing images and minifying CSS/JavaScript.",
      );
    }

    if (score.accessibility < 70) {
      recommendations.push(
        "Improve accessibility by adding alt text to images and proper heading structure.",
      );
    }

    if (score.wordpress < 70) {
      recommendations.push(
        "Consider installing WordPress SEO plugins like Yoast or RankMath for better optimization.",
      );
    }
    ...
    if (score.overall < 50) {
      recommendations.push(
        "Focus on fundamental SEO improvements before advanced optimizations.",
      );
    } else if (score.overall < 80) {
      recommendations.push(
        "Good foundation - focus on content quality and technical improvements.",
      );
    } else {
      recommendations.push(
        "Excellent SEO foundation - consider advanced optimizations and content strategy.",
      );
    }
  ...
  private extractWordPressInfo(pages: CrawlResult[]): {
    ...
    const seoPlugins = Array.from(allPlugins).filter(
      (plugin) =>
