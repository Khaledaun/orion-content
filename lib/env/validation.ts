...
class EnvironmentValidator {
  ...
  validate(): { success: boolean; config?: EnvConfig; errors: string[]; warnings: string[] } {
    try {
  ...
  private performAdditionalValidation() {
    ...
    if (this.config.NODE_ENV === 'production' || this.config.VERCEL) {
      ...
      if (this.config.NEXTAUTH_SECRET === 'demo-secret-please-set-in-production') {
        this.errors.push('Demo NEXTAUTH_SECRET cannot be used in production');
      }
    }

    // OAuth provider validation
    const hasGoogleOAuth = this.config.GOOGLE_CLIENT_ID && this.config.GOOGLE_CLIENT_SECRET;
    const hasGitHubOAuth = this.config.GITHUB_CLIENT_ID && this.config.GITHUB_CLIENT_SECRET;
    ...
    const hasRedis = this.config.REDIS_URL || (this.config.UPSTASH_REDIS_URL && this.config.UPSTASH_REDIS_TOKEN);
    if (!hasRedis) {
      this.warnings.push('No Redis configuration found - sessions will use JWT only');
    }
    ...
    if (this.config.ENABLE_2FA) {
      ...
      if (!hasSmtp) {
        this.warnings.push('2FA is enabled but SMTP is not configured - email-based 2FA will not work');
      }
    }

    // Rate limiting validation
    if (this.config.RATE_LIMIT_REQUESTS > 1000) {
      this.warnings.push('Rate limit is set very high - consider lowering for better security');
    }
  }
