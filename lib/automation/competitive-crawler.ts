...
interface CompetitiveInsight {
  type: 'content_gap' | 'keyword_opportunity' | 'trending_topic' | 'seo_improvement'
...
class CompetitiveCrawler {
  ...
  async addCompetitor(competitor: Omit<CompetitorSite, 'id'>): Promise<string> {
    ...
    logger.info(`Added new competitor: ${competitor.name}`, { id, url: competitor.url })
    return id
  }
  ...
  async crawlCompetitor(competitorId: string): Promise<CrawlResult | null> {
    ...
    try {
      ...
      logger.error(`Failed to crawl competitor ${competitor.name}`, { error, competitorId })
      return null
    }
  }

  private async simulateCrawl(competitor: CompetitorSite): Promise<CrawlResult> {
    ...
    return {
      ...
      seo: {
        ...
        structuredData: [
          { '@type': 'Organization', name: competitor.name }
        ],
  ...
  generateCompetitiveInsights(): CompetitiveInsight[] {
    ...
    allResults.forEach(result => {
      result.content.keywords.forEach(keyword => competitorKeywords.add(keyword))
    })
    ...
    insights.push({
      ...
      actionable: 'Consider creating content targeting these high-performing keywords',
      data: { keywords: Array.from(competitorKeywords).slice(0, 10) }
    })

    // Analyze content length trends
    const avgWordCount = allResults.reduce((sum, result) => sum + result.content.wordCount, 0) / allResults.length
    ...
    insights.push({
      ...
      actionable: 'Consider adjusting content length to match or exceed competitor standards',
      data: { averageWordCount: avgWordCount }
    })

    // Analyze SEO performance
    const mobileOptimizedCount = allResults.filter(result => result.seo.mobileOptimized).length
    ...
    if (mobileOptimizationRate > 0.8) {
      insights.push({
        ...
        actionable: 'Ensure all your content is fully mobile-optimized to remain competitive',
