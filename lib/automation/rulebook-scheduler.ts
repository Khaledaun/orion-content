...
class RulebookScheduler {
  ...
  private initializeScheduler() {
    ...
    logger.info('Rulebook scheduler initialized - bi-weekly updates on even weeks')
  }
  ...
  async scheduleRulebookUpdate(siteId: string, changes: RulebookUpdate['changes']): Promise<string> {
    ...
    logger.info(`Scheduled rulebook update for site ${siteId}`, {
      ...
      changesCount: changes.added.length + changes.modified.length + changes.removed.length
    })
  ...
  private async processBiWeeklyUpdates() {
    ...
    try {
      ...
      logger.info(`Processing ${pendingUpdates.length} pending rulebook updates`)
  ...
  private async processRulebookUpdate(update: RulebookUpdate) {
    try {
      ...
      logger.info(`Processing rulebook update ${update.id} for site ${update.siteId}`)
  ...
  private async applyRulebookChanges(siteId: string, changes: RulebookUpdate['changes']) {
