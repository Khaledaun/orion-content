...
export class DatabaseConnectionManager extends EventEmitter {
  ...
  private connectionStats: Map<string, { count: number; lastUsed: Date }> = new Map()
  ...
  public async getConnection(connectionId: string = 'default'): Promise<PrismaClient> {
  ...
  private updateConnectionStats(connectionId: string, startTime: number): void {
    const stats = this.connectionStats.get(connectionId) || { count: 0, lastUsed: new Date() }
    stats.count++
