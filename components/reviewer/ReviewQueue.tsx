...
export function ReviewQueue({ userRole }: ReviewQueueProps) {
  ...
  const getStatusBadge = (status: string, violationsCount: number) => {
    switch (status) {
      case "NEEDS_REVIEW":
        return (
          <Badge className="bg-orion-amber text-white">Needs Review</Badge>
        );
      case "APPROVED":
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {item.violationsCount > 0 && (
                    <Badge
                      variant="outline"
                      className="text-orion-crimson border-orion-crimson"
                    >
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                {item.lastReview && (
                  <span className="flex items-center gap-1">
                    ...
                    Last reviewed{" "}
                    {new Date(item.lastReview.createdAt).toLocaleDateString()}
                  </span>
                )}
              </div>
              ...
              <Dialog>
                ...
                <DialogContent className="sm:max-w-2xl">
                  <DialogHeader>
                    <DialogTitle className="text-orion-navy">
                      {selectedDraft?.title}
                    </DialogTitle>
                  </DialogHeader>
                      ...
                      <div className="bg-orion-background-light p-4 rounded-lg">
                        <div className="grid grid-cols-2 gap-4 mb-3">
                          <div>
                            ...
                            <div
                              className={`inline-block px-2 py-1 rounded text-sm font-semibold ml-2 ${getScoreColor(selectedDraft.score)}`}
                            >
                              {selectedDraft.score
                                ? `${(selectedDraft.score * 100).toFixed(0)}%`
                                : "N/A"}
                            </div>
                          </div>
                          <div>
                            ...
                            <Badge
                              variant="outline"
                              className="ml-2 text-orion-crimson border-orion-crimson"
                            >
                        ...
                        {selectedDraft.violations &&
                          Array.isArray(selectedDraft.violations) &&
                          selectedDraft.violations.length > 0 && (
                            <div>
                              <p className="text-sm font-semibold text-orion-navy mb-2">
                                Issues Found:
                              </p>
                              <ul className="text-sm text-gray-700 space-y-1">
                                {selectedDraft.violations.map(
                      ...
                      <div className="flex gap-3 justify-end">
                        {userRole !== "VIEWER" && (
                          <>
                            <Button
                              variant="outline"
                              onClick={() =>
                                handleAction(selectedDraft.id, "REJECT")
                              }
                        ...
                        {userRole === "ADMIN" && (
                          <Button
                            variant="outline"
                            onClick={() =>
                              handleAction(selectedDraft.id, "BYPASS")
                            }
                            disabled={actionLoading}
