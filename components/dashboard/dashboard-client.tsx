...
import { LoadingState, ErrorState, EmptyState } from '@/components/ui/enhanced-states';
...
export function DashboardClient({ initialData, userName = 'Admin' }: DashboardClientProps) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <Card className="hover:shadow-lg transition-shadow">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{dict.dashboard.activeSites}</CardTitle>
                <Globe className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-blue-600">{data.siteCount}</div>
                <p className="text-xs text-muted-foreground">
                  {data.siteCount > 0 ? '+2 from last month' : 'Create your first site'}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="hover:shadow-lg transition-shadow">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{dict.dashboard.pendingWeeks}</CardTitle>
                <Calendar className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-green-600">{data.weekCount}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card className="hover:shadow-lg transition-shadow">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{dict.dashboard.totalTopics}</CardTitle>
                <FileText className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-orange-600">{data.topicCount}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-purple-600">{data.jobRunCount}</div>
                <p className="text-xs text-muted-foreground">
                  {data.jobRunCount > 0 ? 'Last run: 2 hours ago' : 'No jobs yet'}
                </p>
              </CardContent>
            </Card>
          </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">{dict.dashboard.throughput}</span>
                      </div>
                      <div className="text-lg font-bold">{data.metrics.throughput.toLocaleString()}/mo</div>
                    </div>
                    
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <CheckCircle className="h-4 w-4 text-blue-600" />
                        <span className="text-sm font-medium">{dict.dashboard.qaPassRate}</span>
                      </div>
                      <div className={`text-lg font-bold ${getPassRateColor(data.metrics.qaPassRate)}`}>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">{dict.dashboard.avgLatency}</span>
                      </div>
                      <div className="text-lg font-bold">{formatLatency(data.metrics.avgLatency)}</div>
                    </div>
                    
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <DollarSign className="h-4 w-4 text-purple-600" />
                        <span className="text-sm font-medium">{dict.dashboard.costPerMonth}</span>
                      </div>
                      <div className="text-lg font-bold">${data.metrics.costPerMonth}</div>
                    </div>
                  </div>
                ) : (
                  <EmptyState 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Database & files</p>
                  </div>
                  <Badge variant="default" className="bg-green-100 text-green-800">
                    Operational
                  </Badge>
