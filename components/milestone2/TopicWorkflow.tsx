...
export function TopicWorkflow({ 
  ...
  const filteredTopics = selectedNiche === 'all' 
    ? topics 
    : topics.filter(topic => topic.niche === selectedNiche);

  const approvedCount = topics.filter(t => t.status === 'approved').length;
  const pendingCount = topics.filter(t => t.status === 'pending').length;
  const avgQualityScore = Math.round(topics.reduce((sum, t) => sum + t.qualityScore, 0) / topics.length);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            AI-powered topic generation with playful A/B testing. Let's create content that converts! 🎯
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-l-4 border-l-green-500">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Approved Topics</p>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{avgQualityScore}%</p>
              </div>
        ...
        <CardContent className="p-4">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <Label className="text-sm font-medium">Topic Generation Progress</Label>
              <span className="text-sm text-gray-600">{approvedCount} / {maxTopics} topics approved</span>
            </div>
            <Progress value={(approvedCount / maxTopics) * 100} className="h-3" />
            <p className="text-xs text-gray-500">
              {maxTopics - approvedCount} more topics needed to complete your content calendar
            </p>
          </div>
        </CardContent>
      </Card>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600 mb-3 italic">"{topic.angle}"</p>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-sm">{topic.searchVolume.toLocaleString()}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-medium text-sm text-green-600">{topic.qualityScore}%</p>
                        </div>
                        <div>
                          <Label className="text-xs text-gray-500">Engagement Prediction</Label>
                          <p className="font-medium text-sm text-blue-600">{topic.engagementPrediction}%</p>
                        </div>
                      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-white rounded-lg border border-purple-200">
                <h4 className="font-semibold text-gray-900 mb-2">🅰️ Variant A Performance</h4>
                <div className="space-y-2">
              ...
              <div className="p-4 bg-white rounded-lg border border-purple-200">
                <h4 className="font-semibold text-gray-900 mb-2">🅱️ Variant B Performance</h4>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-sm">Click-through Rate</span>
            ...
            <div className="mt-4 p-3 bg-green-100 rounded-lg border border-green-200">
              <p className="text-sm text-green-800">
                🎉 <strong>Winner:</strong> Variant B is performing 28% better! Consider using similar angles for future topics.
              </p>
            </div>
          </CardContent>
        </Card>
