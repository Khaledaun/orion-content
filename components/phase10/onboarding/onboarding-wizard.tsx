...
import { SkipLink, LiveRegion, useFocusManagement } from '@/components/ui/accessibility'
...
export function OnboardingWizard() {
  ...
  useEffect(() => {
    if (status && !loading) {
      ...
      announcePage(`${dict.onboarding.step} ${currentStep + 1} ${dict.onboarding.of} ${STEPS.length}: ${currentStepConfig.title}`)
    }
  ...
  const handleStepComplete = async (stepId: string, data?: any) => {
    ...
    try {
      ...
      toast.success(`${STEPS[currentStep].title} ${dict.onboarding.completed}!`)
      ...
      >
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              {status.progress.completed} {dict.onboarding.of} {status.progress.total} {dict.onboarding.completed}
            </Badge>
          </div>
          ...
          <div className="mb-6">
            ...
            <LiveRegion level="polite" className="sr-only">
              {dict.onboarding.progress}: {status.progress.percentage}% {dict.onboarding.completed}
            </LiveRegion>
          </div>
                  ...
                  <div className={`flex items-center space-x-2 ${
                    ...
                    {isCompleted ? (
                      ...
                      <Circle 
                        ...
                        aria-label={isCurrent ? `${dict.onboarding.step} ${index + 1}: ${step.title}` : step.title}
                      />
                    )}
