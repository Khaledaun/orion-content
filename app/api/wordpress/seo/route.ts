  ...
  try {
    ...
    switch (action) {
      ...
      case 'auto_fill_post': {
        ...
        const pluginConfig = await formAutoFill.detectWordPressPlugins();
        const result = await formAutoFill.autoFillPost(postId, formData, pluginConfig);
      ...
      case 'suggest_categories': {
        ...
        const suggestions = await formAutoFill.suggestCategories(content, keywords);
