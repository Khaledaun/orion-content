  ...
  try {
    ...
    switch (action) {
      case 'setup_monitoring': {
        const { domain, competitors, monitoringFrequency, alertThresholds, enabledAlerts, notificationChannels, webhookUrl } = data;
        ...
        const config = await competitorMonitor.setupCompetitorMonitoring({
          ...
          enabledAlerts: enabledAlerts || ['new_keyword', 'position_change', 'new_backlink'],
        ...
        logger.info(
          { userId, siteId, domain: redactSensitive(domain), competitorsCount: competitors.length },
      ...
      case 'analyze_competitors': {
        ...
        const profiles = await competitorMonitor.analyzeCompetitors(domain, competitors);
      ...
      case 'compare_competitors': {
        ...
        const comparison = await competitorMonitor.compareWithCompetitors(domain, competitors);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'alerts': {
        ...
        const alerts = await competitorMonitor.getCompetitorAlerts(siteId, limit);
      ...
      case 'trends': {
        ...
        const trends = await competitorMonitor.getCompetitorTrends(siteId, competitor, days);
