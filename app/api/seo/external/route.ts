...
import { ExternalSEOAPIManager, SubscriptionTier } from '@/lib/seo/external-apis';
  ...
  try {
    ...
    switch (action) {
      case 'keyword_analysis': {
        ...
        logger.info(
          { userId, siteId, keywords: redactSensitive(keywords), provider, userTier },
          'Starting keyword analysis'
        );

        const keywordData = await apiManager.getKeywordData(keywords, provider, userTier);
        ...
        await prisma.seoSiteAudit.create({
          data: {
            ...
            summary: {
              ...
              averageVolume: keywordData.reduce((sum, k) => sum + k.searchVolume, 0) / keywordData.length,
              averageDifficulty: keywordData.reduce((sum, k) => sum + k.difficulty, 0) / keywordData.length,
            },
          },
        });
      ...
      case 'competitor_analysis': {
        ...
        logger.info(
          { userId, siteId, domain: redactSensitive(domain), provider, userTier },
          'Starting competitor analysis'
        );

        const competitorData = await apiManager.getCompetitorData(domain, provider, userTier);
      ...
      case 'subscription_info': {
        return NextResponse.json({
          ...
          subscription: {
            tier: userTier,
            limits: limits,
            canAccessExternalAPIs: subscriptionManager.canAccessExternalAPIs(userTier),
            canAccessBacklinkAnalysis: subscriptionManager.canAccessBacklinkAnalysis(userTier),
            canAccessCompetitorAnalysis: subscriptionManager.canAccessCompetitorAnalysis(userTier),
          },
...
function determineSubscriptionTier(userRoles: any[]): SubscriptionTier {
  ...
  const hasProRole = userRoles.some(role => 
  ...
  const hasGuruRole = userRoles.some(role => 
    role.role.name.toLowerCase().includes('guru') ||
    role.role.name.toLowerCase().includes('enterprise')
  );
