...
async function handler(req: NextRequest) {
  ...
  try {
    ...
    if (!owner || !repo) {
      return NextResponse.json(
        { error: "Invalid GITHUB_REPO_FULL format" },
        { status: 400 },
      );
    }
    ...
    if (connectionMap.has("openai")) {
      const data = decryptJson<{ apiKey: string }>(
        connectionMap.get("openai")!.dataEnc,
      );
      secretsToWrite.push({ name: "OPENAI_API_KEY", value: data.apiKey });
    }

    if (connectionMap.has("search_cse")) {
      const data = decryptJson<{ id: string; key: string }>(
        connectionMap.get("search_cse")!.dataEnc,
      );
    ...
    if (connectionMap.has("search_bing")) {
      const data = decryptJson<{ key: string }>(
        connectionMap.get("search_bing")!.dataEnc,
      );
    ...
    if (connectionMap.has("console_api_token")) {
      const data = decryptJson<{ token: string }>(
        connectionMap.get("console_api_token")!.dataEnc,
      );
    ...
    for (const site of sites) {
      ...
      if (connectionMap.has(wpKind)) {
        ...
        secretsToWrite.push({
          name: `WP_BASE_URL_${site.key.toUpperCase()}`,
          value: data.baseUrl,
        });
        secretsToWrite.push({
          name: `WP_USER_${site.key.toUpperCase()}`,
          value: data.user,
        });
        secretsToWrite.push({
          name: `WP_APP_PASSWORD_${site.key.toUpperCase()}`,
    ...
    if (process.env.CONSOLE_BASE_URL) {
      secretsToWrite.push({
        ...
        value: process.env.CONSOLE_BASE_URL,
      });
    }
