...
import { Trash2, Plus, Eye, EyeOff, Settings, Wifi, WifiOff } from 'lucide-react';
...
import { StateIndicator, getConnectionState, type ConnectionState } from '@/components/ui/state-indicator';
...
export default function CredentialsPage() {
  ...
  useEffect(() => {
    ...
    const enhancedCredentials: EnhancedStoredCredential[] = loadedCredentials.map(cred => ({
      ...cred,
  ...
  const testCredential = async (credentialId: string) => {
    ...
    try {
      ...
      toast.success(isSuccess ? 'Connection test successful' : 'Connection test failed');
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name || !form.data || !encryptionKey) {
      toast.error(`${dict.credentials.name} and ${dict.credentials.data} are required`);
      return;
    }
    ...
    try {
      ...
      const enhancedCredentials: EnhancedStoredCredential[] = loadedCredentials.map(cred => ({
        ...cred,
        connectionState: cred.id === credential.id ? 'unknown' : (credentials.find(c => c.id === cred.id)?.connectionState || 'unknown'),
        isActive: true
      }));
      setCredentials(enhancedCredentials);
  ...
  const handleDelete = (id: string) => {
    if (confirm(`${dict.common.delete} credential? This action cannot be undone.`)) {
      removeCredential(id);
      const loadedCredentials = getStoredCredentials();
      const enhancedCredentials: EnhancedStoredCredential[] = loadedCredentials.map(cred => ({
        ...cred,
        connectionState: credentials.find(c => c.id === cred.id)?.connectionState || 'unknown',
        isActive: true
      }));
      setCredentials(enhancedCredentials);
  ...
  const handleDecrypt = async (credential: StoredCredential) => {
    try {
      ...
      toast.success(`Decrypted: ${decrypted.substring(0, 50)}${decrypted.length > 50 ? '...' : ''}`);
    } catch (error) {
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Button type="submit" disabled={isLoading}>
                ...
                {isLoading ? dict.common.loading : `${dict.common.create} ${dict.credentials.name}`}
              </Button>
            </form>
          </CardContent>
        </Card>
                  ...
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <StateIndicator 
                          state={testingCredentials.has(credential.id) ? 'loading' : (credential.connectionState || 'unknown')}
                          size="sm"
                        />
                      </div>
                      <p className="text-sm text-muted-foreground">
                        {dict.credentials.type}: {credential.type} • Created: {new Date(credential.createdAt || Date.now()).toLocaleDateString()}
                        {credential.lastTested && (
                          <> • Last tested: {credential.lastTested.toLocaleDateString()}</>
                        )}
                      </p>
                    </div>
                    <div className="flex items-center space-x-2">
