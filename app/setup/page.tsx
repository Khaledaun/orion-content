...
export default function SetupPage() {
  ...
  const handleCreateSite = async () => {
    if (!siteData.key || !siteData.name) {
      toast.error(
        `${dict.setup.siteKey} and ${dict.setup.siteName} are required`,
      );
      return;
    }
  ...
  const handleSaveSecrets = async () => {
    ...
    try {
      ...
      if (
        ...
        connectorData.searchCse?.id &&
        connectorData.searchCse?.key
      ) {
        const response = await fetch("/api/setup/secrets", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      // Save WordPress
      if (
        connectorData.wordpress?.baseUrl &&
        connectorData.wordpress?.user &&
        connectorData.wordpress?.appPassword
      ) {
        const response = await fetch("/api/setup/secrets", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        <main id="main-content" role="main" tabIndex={-1}>
          ...
          >
            <TabsList className="grid w-full grid-cols-3" role="tablist">
              <TabsTrigger value="site" role="tab">
                {dict.setup.siteSetup}
              </TabsTrigger>
              <TabsTrigger value="connectors" role="tab">
                {dict.setup.connectors}
              </TabsTrigger>
              <TabsTrigger value="github" role="tab">
                {dict.setup.githubSecrets}
              </TabsTrigger>
            </TabsList>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          Generate a token for pipeline access
                        </p>
                      </div>
                      <Button
                  ...
                  {secrets.length > 0 && (
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        {secrets.map((secret) => (
                          <li key={secret} className="text-sm text-gray-600">
                            {secret}
                          </li>
                        ))}
                      </ul>
                  ...
                  {githubResult && (
                    <div className="mt-4 p-4 bg-green-50 rounded-md">
                      <h3 className="font-medium text-green-800">
                        Successfully wrote {githubResult.written} secrets:
                      </h3>
